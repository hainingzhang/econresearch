...
当前项目实际运行以下七个步骤，对应 `src/` 目录中的七个顺序脚本：
...
对一个完整的 DID 研究，建议在描述性分析与回归估计之间加入“识别假设评估”步骤。对应的推荐流程为：

1. 数据下载
2. 数据清洗
3. 数据审计
4. 描述性分析
5. 识别假设评估（建议脚本名：`04.5_identification_assessment.py`）
6. DID 回归
7. 稳健性检验
8. 撰写研究备忘录
...
本次研究**没有实施正式的识别假设评估脚本**。原因是当前数据只有一个政策前时期，不能真正运行正式的平行趋势检验，也无法以多期事件研究来评估政策前动态。因此，现有流程会在研究计划和备忘录中说明识别假设与局限，但不应将其视为已经完成的假设检验。未来在具备多个政策前时期或更丰富的辅助资料时，可新增 `04.5_identification_assessment.py` 来系统记录上述评估。
